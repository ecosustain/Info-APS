...
from core.models.data_import import DataImportProfile, DataImportParam, DataImportCategory
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        data_import_category = DataImportCategory.objects.get(name='import_data_file')
        ...
        for attribute_name, attribute_data in data_class[json_node_name].items():
            if DataImportProfile.objects.filter(name=attribute_name).exists():
                data_import_profile = DataImportProfile.objects.get(name=attribute_name)
            ...
            if data_import_profile:
                for params in attribute_data['params']:
                    ...
                    data_import_param, created = DataImportParam.objects.get_or_create(
                        name="file_import_param",
                        value=extra_data,
                        data_import_profile=data_import_profile
                    )

            self.stdout.write(self.style.SUCCESS(f"Imported DataImportProfiles {data_import_profile.name}"))
