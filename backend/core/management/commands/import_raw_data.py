...
def add_or_get_category(name):
    try:
        ...
        category, created = IndividualCategory.objects.get_or_create(name=name)
        if created:
        ...
        print(
            f"Categoria '{name}' já existe. Usando a categoria existente. {e}"
        )
...
def load_data(file_path):
    ...
    if file_path.endswith(".xlsx") or file_path.endswith(".xls"):
        ...
        df = pd.read_csv(
            file_path, delimiter=",", low_memory=False
        )  # Assumindo que o separador é uma vírgula
    else:
        raise ValueError(
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for _, row in df.iterrows():
            ...
            ibge_code = int(
                row["Ibge"]
            )  # Supondo que o código IBGE esteja na coluna "IBGE"
            try:
                with transaction.atomic():
                    ...
                    if ibge_code:
                        ...
                        geo_unit.save()  # Salva as mudanças no banco de dados

                    individual_care = IndividualCare.objects.create(
                    ...
                    for col in df.columns:
                        if col not in ignored_columns and not pd.isna(
                            row[col]
                        ):
                            if type(row[col]) == str:
                            ...
                            if not pd.isna(row[col]) and value != 0:
                                ...
                                count = IndividualCare.objects.filter(
                                    ...
                                    month=month,
                                    geo_unit=geo_unit,
                                    individualcarecategory__individual_category__name=individual_category[
                                        col
                                    ],
                                ).count()
                                if count == 0:
                                    IndividualCareCategory.objects.create(
                                        individual_care=individual_care,
