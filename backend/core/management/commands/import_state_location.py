    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():  # Usa transações para garantir consistência
            for index, row in df.iterrows():
                ...
                geo_units = GeoUnit.objects.filter(name=row['UF'], type_id='Estado')  # Filtra primeiro pela UF
                ...
                for geo_unit in geo_units:
                    ...
                    print(f'Não econtrado: {updated_count}/{len(df)}: {uf_excel} - {latitude}, {longitude}')
