...
def move_and_rename_file(file_name):
    ...
    destination_path = os.path.join(
        destination_folder, file_name_with_timestamp
    )
    ...
    try:
        ...
        print(
            f"Arquivo movido com sucesso para {destination_folder} com o nome {file_name_with_timestamp}."
        )
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # TODO: Para carregar dados de uma pasta, hibilitar o código abaixo. As configurações são carregadas no banco a partir do arquivo seed_json/data_import_profile.json.
        #  Nele, há o caminho onde devem esta localizado os dados que devem ser baixados de https://dlab-public-files.s3.us-east-2.amazonaws.com/sisab-data-dist.zip
...
def process_file(param, data_import_profiles):
    ...
    if os.path.isfile(file_path):
        try:
            ...
            call_command(
                "import_raw_data", group_param, class_param, file_path
            )
...
def run_import(data_import_profiles):
    active_params = data_import_profiles.params.filter(
        active=True, name="file_import_param"
    )
