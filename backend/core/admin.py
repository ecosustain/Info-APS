    ...
    def queryset(self, request, queryset):
        ...
        if self.value():
            return queryset.filter(
                individualcarecategory__individual_category_id=self.value()
            )
        return queryset
...
class IndividualCareAdmin(admin.ModelAdmin):
    ...
    list_filter = [
        CategoryFilter
    ]  # Adiciona o filtro personalizado por categoria

    def get_categories(self, obj):
        """Exibe as categorias relacionadas a um cuidado individual."""
        return ", ".join(
            [
                str(category.individual_category.name)
                for category in obj.individualcarecategory_set.all()
            ]
        )
