...
ns_files = Namespace("Sistema e Arquivos", description="Operações de apoio a operação do sistema")
...
class FileDownload(Resource):
    ...
    def post(self):
        ...
        if cities:
            cities = [int(city) for city in cities]  # Converte cada elemento de cities para inteiro
            query['Ibge'] = {"$in": cities}  # Se a lista de municípios não estiver vazia

        step += 1
        progress_manager.set_progress(progress_id, int((step / total_records) * 100), f'{step}/{total_records}')
        try:
    ...
    def post(self):
        ...
        try:
            ...
            if file and file.filename.endswith('.csv'):
                ...
                total_imported = process_csv_file(file, collection_name, progress_id)
