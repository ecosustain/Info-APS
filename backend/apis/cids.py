...
def agregacao_por_cid(collection, rows):
    ...
    no_count = ["CIAP (N01) Cefaléia", "CIAP (R05) Tosse", "Ano", "Mes", "Uf", "Ibge", "Municipio"]

    for row in rows:
        year, month = row['_id']['Ano'], row['_id']['Mes']
        cid_total = sum(
            int(row[attribute]) for attribute in collection_attributes if attribute not in no_count)
