...
def create_mongo_collection(collection_name):
    ...
    if collection_name in collections:
        return False, f"A coleção '{collection_name}' já existe no banco de dados."
    ...
    for document in documents:
        for key, value in document.items():
            if key not in ignore_keys and isinstance(value, (int, float)):
                collection_summary[key] = collection_summary.get(key, 0) + value
            elif isinstance(value, dict):
                for sub_key, sub_value in value.items():
                    if isinstance(sub_value, (int, float)):
                        key_composta = f"{key}.{sub_key}"
                        collection_summary[key_composta] = collection_summary.get(key_composta, 0) + sub_value
...
def get_collection_attributes(collection_name):
    ...
    result = db['collection_attributes'].find_one(
        ...
        {'_id': 0, 'attributes': 1}  # Excluir o campo '_id', retornar apenas os atributos
    )
