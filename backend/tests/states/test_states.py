...
from apis.states import State, StateSumByCollection, StateSumByCollectionYear  # Módulo 'api'
...
class StateTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.api.add_resource(StateSumByCollection, '/states/sum/<string:state>')
        self.api.add_resource(StateSumByCollectionYear, '/states/sum/<string:state>/<int:year>')
    ...
    def test_get_states_success(self, mock_get_state):
        ...
        mock_get_state.return_value = [{'name': 'California'}, {'name': 'Texas'}]

        response = self.client.get('/states')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, [{'name': 'California'}, {'name': 'Texas'}])
    ...
    def test_get_collection_sum_by_state_success(self, mock_get_collection_sum_states):
        """Teste para verificar o sucesso ao buscar soma de coleções por estado"""
        mock_get_collection_sum_states.return_value = [{'state': 'California', 'sum': 1500}]
    ...
    @patch('api.states.get_collection_sum_states')
    def test_get_collection_sum_by_state_failure(self, mock_get_collection_sum_states):
        """Teste para verificar falha ao buscar soma de coleções por estado"""
        mock_get_collection_sum_states.side_effect = Exception('Collection sum error')
    ...
    def test_get_collection_sum_by_state_year_success(self, mock_get_collection_sum_states_year):
        """Teste para verificar o sucesso ao buscar soma de coleções por estado e ano"""
        mock_get_collection_sum_states_year.return_value = [{'state': 'California', 'year': 2021, 'sum': 2500}]

        response = self.client.get('/states/sum/California/2021')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, [{'state': 'California', 'year': 2021, 'sum': 2500}])

    @patch('api.states.get_collection_sum_states_year')
    def test_get_collection_sum_by_state_year_failure(self, mock_get_collection_sum_states_year):
        """Teste para verificar falha ao buscar soma de coleções por estado e ano"""
        mock_get_collection_sum_states_year.side_effect = Exception('Yearly collection sum error')
