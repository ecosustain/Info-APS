...
from apis.collections import Collection, CollectionSum, CollectionSumByKey  # Módulo 'api'
...
class CollectionTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.api.add_resource(CollectionSumByKey, '/collections/<collection>/<attribute>')
    ...
    def test_get_all_collections_success(self, mock_get_all_collections):
        ...
        mock_get_all_collections.return_value = [{'name': 'Collection1'}, {'name': 'Collection2'}]

        response = self.client.get('/collections')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json, [{'name': 'Collection1'}, {'name': 'Collection2'}])
    ...
    def test_get_all_collections_failure(self, mock_get_all_collections):
        ...
        mock_get_all_collections.side_effect = Exception('Erro ao buscar coleções')
    ...
    def test_get_collection_sum_failure(self, mock_get_collection_sum):
        ...
        mock_get_collection_sum.side_effect = Exception('Erro ao buscar soma da coleção')
...
# Testes para a classe CollectionSumByKey
    ...
    def test_get_collection_sum_by_key_failure(self, mock_get_collection_sum):
        """Teste para verificar falha ao buscar soma de uma coleção"""
        mock_get_collection_sum.side_effect = Exception('Internal Server Error')
