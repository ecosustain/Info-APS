...
def test_sum_enfermeiro_for_araras():
    ...
    result = df[(df["Municipio"] == "ARARAS") & (df["Ano"] == ANO)]["Enfermeiro"].sum()
...
def test_sum_medico_for_araras():
    ...
    result = df[(df["Municipio"] == "ARARAS") & (df["Ano"] == ANO)]["Médico"].sum()
...
def test_sum_nutricionista_for_araras():
    ...
    result = df[(df["Municipio"] == "ARARAS") & (df["Ano"] == ANO)]["Nutricionista"].sum()
    expected_value = 41
    assert result == expected_value, f"Esperado {expected_value}, mas obteve {result}"
