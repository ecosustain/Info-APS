...
def test_get_atendimentos_brasil():
    ...
    assert sum(sum(anos['2023'].values()) for profissional, anos in result.items() if '2023' in anos) == 356894032
...
def test_get_atendimentos_sp():
    ...
    assert sum(sum(anos['2023'].values()) for profissional, anos in result.items() if '2023' in anos) == 74981068
