...
ns_altas = Namespace("Altas", description="Operações sobre os atributos da coleção Produção Conduta")
...
def agregacao_por_altas(collection, rows):
    ...
    for row in rows:
        ...
        highs_total = sum(int(row[attribute]) for attribute in attributes if
                          attribute == "Alta do episódio")
...
@ns_altas.route('/v1/altas/states/<state>', strict_slashes=False)
...
@ns_altas.route('/v1/altas/cities/<ibge>', strict_slashes=False)
